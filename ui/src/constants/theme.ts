export const THEME_COLORS = {
  FORMAT: '#ef502b',
  STAR: '#ffce00',
  TITLE: '#c48e4e',
  AUTHOR: '#c48e4e',
  AUTHOR_FOCUS: '#69afc6',
  DESCRIPTION: '#666666',
  FOCUS_BOOK: '#3c485a',
  FOCUS_BOOK_DARK: '#5c6b7d',
  MENU_ACTIVE: '#0c858b',
  SHELF_ICON: '#69afc6',
  SHELF_ICON_DARK: '#8a9ba8',
  SHELF_BEIGE_SHADOW_LIGHT: 'rgba(196, 142, 78, 0.25)',
  SHELF_BEIGE_SHADOW_LIGHTER: 'rgba(196, 142, 78, 0.15)',
  SHELF_BEIGE_SHADOW_MEDIUM: 'rgba(196, 142, 78, 0.35)',
  SHELF_BEIGE_SHADOW_HEAVY: 'rgba(196, 142, 78, 0.25)',
  BGD_1: '#e8f6ff',
  BGD_1_DARK: '#2a3a45',
  BGD_2: '#fcefe3',
  BGD_2_DARK: '#3b3229',
  BGD_3_FILL: '#f9f9f9',
  BGD_3_FILL_DARK: '#2a2a2a',
  BGD_3_OUTLINE: '#e2e2e2',
  BGD_3_OUTLINE_DARK: '#555555',
  TEXT: '#000000',
  TEXT_DARK: '#ffffff',
  GRID: '#cccccc',
  GRID_DARK: '#444444'
} as const

export const LAYOUT = {
  MAX_CONTENT_WIDTH: '1200px',
  HEADER_HEIGHT: '64px',
  FOOTER_HEIGHT: '136px',
  VIEW_PADDING: '2rem 0',
  VIEW_PADDING_MOBILE: '1.5rem 0'
} as const

export const AVATAR_SIZES = {
  CARD: 80,
  CAROUSEL: 64,
  DETAIL: 120,
  TABLET: 100
} as const

export const ICON_SIZES = {
  DETAIL: 48,
  LIST: 32
} as const

export const CAROUSEL_ARROW_SIZES = {
  BUTTON: 40,
  ICON: 28
} as const

export const TYPOGRAPHY = {
  FONT_FAMILY: "'Inter Variable', 'Inter', sans-serif",
  H1_SIZE: '1.5rem',
  H1_SIZE_MOBILE: '1rem',
  H1_WEIGHT: 700,
  H2_SIZE: '1.375rem',
  H2_SIZE_MOBILE: '1rem',
  H2_WEIGHT: 500,
  H3_SIZE: '0.9375rem',
  H3_SIZE_MOBILE: '0.75rem',
  H3_WEIGHT: 600,
  BODY_SIZE: '0.875rem',
  BODY_SIZE_MOBILE: '0.75rem',
  BODY_WEIGHT: 400,
  CAPTION_SIZE: '0.8125rem',
  CAPTION_SIZE_MOBILE: '0.8125rem',
  CAPTION_WEIGHT: 500,
  SMALL_SIZE: '0.75rem',
  SMALL_SIZE_MOBILE: '0.6875rem',
  SMALL_WEIGHT: 500,
  BUTTON_SIZE: '0.875rem',
  BUTTON_SIZE_MOBILE: '0.875rem',
  BUTTON_WEIGHT: 500,
  DESCRIPTION_SIZE: '0.75rem',
  DESCRIPTION_SIZE_MOBILE: '0.625rem',
  DESCRIPTION_WEIGHT: 500
} as const
