export const THEME_COLORS = {
  FORMAT: '#ef502b',
  STAR: '#ffce00',
  TITLE: '#c48e4e',
  AUTHOR: '#c48e4e',
  AUTHOR_FOCUS: '#69afc6',
  DESCRIPTION: '#666666',
  FOCUS_BOOK: '#3c485a',
  FOCUS_BOOK_DARK: '#5c6b7d',
  MENU_ACTIVE: '#0c858b',
  COLLECTION_ICON: '#69afc6',
  COLLECTION_ICON_DARK: '#8a9ba8',
  AUTHOR_AVATAR_BGD: '#69afc6',
  AUTHOR_AVATAR_BGD_DARK: '#8a9ba8',
  COLLECTION_BEIGE_SHADOW_LIGHT: 'rgba(196, 142, 78, 0.25)',
  COLLECTION_BEIGE_SHADOW_LIGHTER: 'rgba(196, 142, 78, 0.15)',
  COLLECTION_BEIGE_SHADOW_MEDIUM: 'rgba(196, 142, 78, 0.35)',
  COLLECTION_BEIGE_SHADOW_HEAVY: 'rgba(196, 142, 78, 0.25)',
  BGD_1: '#e8f6ff',
  BGD_1_DARK: '#2a3a45',
  BGD_2: '#fcefe3',
  BGD_2_DARK: '#3b3229',
  BGD_3_FILL: '#f9f9f9',
  BGD_3_FILL_DARK: '#2a2a2a',
  BGD_3_OUTLINE: '#e2e2e2',
  BGD_3_OUTLINE_DARK: '#555555',
  TEXT: '#000000',
  TEXT_DARK: '#ffffff',
  GRID: '#cccccc',
  GRID_DARK: '#444444'
} as const

export const LAYOUT = {
  MAX_CONTENT_WIDTH: '1200px',
  HEADER_HEIGHT: '64px',
  FOOTER_HEIGHT: '136px',
  VIEW_PADDING_HORIZONTAL: '1.6rem',
  VIEW_PADDING: '2rem 1.6rem',
  VIEW_PADDING_MOBILE: '1.5rem 1.6rem',
  CARD_BORDER: '1.5px'
} as const

export const AVATAR_SIZES = {
  COMPACT: 48,
  COMFORTABLE: 100,
  FULL: 160
} as const

export const ICON_SIZES = {
  COMPACT: 28,
  COMFORTABLE: 80,
  FULL: 100
} as const

export const BOOK_GRID = {
  default: { width: 190, widthMobile: 160, coverHeight: 190, coverHeightMobile: 145 },
  wide: { width: 214, widthMobile: 160, coverHeight: 190, coverHeightMobile: 145 },
  compact: { width: 160, widthMobile: 160, coverHeight: 145, coverHeightMobile: 145 }
} as const

export type BookGridVariant = keyof typeof BOOK_GRID

export const CAROUSEL_ARROW_SIZES = {
  BUTTON: 40,
  ICON: 28
} as const

export const TYPOGRAPHY = {
  FONT_FAMILY: "'Inter Variable', 'Inter', sans-serif",
  H1_SIZE: '1.5rem',
  H1_SIZE_MOBILE: '1rem',
  H1_WEIGHT: 700,
  H2_SIZE: '1.375rem',
  H2_SIZE_MOBILE: '1rem',
  H2_WEIGHT: 500,
  H3_SIZE: '0.9375rem',
  H3_SIZE_MOBILE: '0.75rem',
  H3_WEIGHT: 600,
  H4_SIZE: '0.75rem',
  H4_SIZE_MOBILE: '0.65rem',
  H4_WEIGHT: 500,
  BODY_SIZE: '0.875rem',
  BODY_SIZE_MOBILE: '0.75rem',
  BODY_WEIGHT: 400,
  CAPTION_SIZE: '0.8125rem',
  CAPTION_SIZE_MOBILE: '0.8125rem',
  CAPTION_WEIGHT: 500,
  SMALL_SIZE: '0.75rem',
  SMALL_SIZE_MOBILE: '0.6875rem',
  SMALL_WEIGHT: 500,
  BUTTON_SIZE: '0.875rem',
  BUTTON_SIZE_MOBILE: '0.875rem',
  BUTTON_WEIGHT: 500,
  DESCRIPTION_SIZE: '0.90rem',
  DESCRIPTION_SIZE_MOBILE: '0.625rem',
  DESCRIPTION_WEIGHT: 500
} as const
