 ...
 * Only includes catalog language codes used by the bundled sources and UI locales
...
// Catalog language codes included in the generated library UI.
const CATALOG_LANGUAGE_CODES = [
...
// Some catalog codes use ISO 639-3 where CLDR has an ISO 639-1 key.
const CATALOG_CODE_TO_CLDR = {
  gla: 'gd', // Scottish Gaelic
  nav: 'nv', // Navajo
  oji: 'oj'  // Ojibwa
}

// Each UI locale has one file; source-specific messages are top-level namespaces.
...
for (const locale of UI_LOCALES) {
  ...
  const localeData = {}
  for (const code of CATALOG_LANGUAGE_CODES) {
    const cldrCode = CATALOG_CODE_TO_CLDR[code] || code
